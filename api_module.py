...
def set_update(request,*args,**kwargs):
	...
	:param: x = user_id % competition. x = 0 인 경우 id = user_id를 가진 사용자는 실험적 업데이트를 받는다. (설정된 경우 경쟁 색인! = 0). 다른 경우 사용자는 표준 업데이트를 받는다. 공식 및 인덱싱에 대한 권한이 있는 사용자가 갱신 요청을 사용할 경우에만 사용된다. 사용자가 인증되지 않은 경우 이 알고리즘은 사용되지 않으며 사용자는 표준 업데이트를 받는다.
